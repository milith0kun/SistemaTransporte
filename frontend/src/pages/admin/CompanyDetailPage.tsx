...
import {
  ...
  Search, RefreshCw, QrCode, Download, Shield, Car,
} from 'lucide-react';
...
function QrMiniModal({ vehicleId, plate, onClose }: { vehicleId: string; plate: string; onClose: () => void }) {
  ...
  return (
    <Modal open={true} onClose={onClose} title={`Código QR — ${plate}`} size="sm">
      <ModalBody className="flex flex-col items-center gap-4">
        {loading
          ? <div className="h-48 flex items-center justify-center"><RefreshCw className="h-8 w-8 text-[#2E86C1] animate-spin" /></div>
          : qrSrc
            ? <img src={qrSrc} alt="QR" className="w-48 h-48 rounded-lg border border-gray-200" />
            : <div className="h-48 flex items-center justify-center"><p className="text-sm text-red-500">Error al cargar QR</p></div>}
      </ModalBody>
      <ModalFooter className="flex gap-2 w-full">
        <Button variant="outline" className="flex-1" onClick={onClose}>Cerrar</Button>
        <Button className="flex-1" onClick={download} disabled={!qrSrc}>
  ...
  if (loadingCompany) {
    return (
      <div className="flex items-center justify-center h-64">
        <RefreshCw className="h-7 w-7 text-[#2E86C1] animate-spin" />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        <button onClick={() => navigate(-1)}
          className="p-1.5 rounded-lg border border-gray-300 hover:bg-gray-50 shrink-0 mt-0.5 transition-colors">
          <ArrowLeft className="h-4 w-4 text-gray-600" />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-xl font-bold text-gray-900">{company.name}</h1>
            <Badge className={`${COMPANY_STATUS_COLORS[company.status]} text-xs`}>{company.status}</Badge>
          </div>
          <p className="text-sm text-gray-500 mt-1">RUC {company.ruc}</p>
        </div>
        {canEdit && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4 shadow-sm">
          <div className="p-3 bg-blue-50 rounded-lg"><Building2 className="h-6 w-6 text-[#2E86C1]" /></div>
          <div>
            <p className="text-sm text-gray-500 font-medium">Reputación</p>
            <p className="text-2xl font-bold text-gray-900">{company.reputation_score}</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4 shadow-sm">
          <div className="p-3 bg-green-50 rounded-lg"><Users className="h-6 w-6 text-green-600" /></div>
          <div>
            <p className="text-sm text-gray-500 font-medium">Conductores</p>
            <p className="text-2xl font-bold text-gray-900">{company.driver_count}</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4 shadow-sm">
          <div className="p-3 bg-purple-50 rounded-lg"><Bus className="h-6 w-6 text-purple-600" /></div>
          <div>
            <p className="text-sm text-gray-500 font-medium">Vehículos</p>
            <p className="text-2xl font-bold text-gray-900">{company.vehicle_count}</p>
          </div>
        </div>
      </div>
          ...
          {(['info', 'drivers', 'vehicles'] as Tab[]).map(t => (
            <button
              ...
              className={`px-6 py-3.5 text-sm font-medium transition-colors border-b-2 ${
                tab === t
                  ? 'text-[#1B4F72] border-[#2E86C1] bg-white'
                  : 'text-gray-500 border-transparent hover:text-gray-700 hover:bg-gray-100'
              }`}
            >
              {t === 'info' ? 'Información General' : t === 'drivers' ? `Conductores (${company.driver_count})` : `Vehículos (${company.vehicle_count})`}
            </button>
          ))}
        </div>
        ...
        {tab === 'drivers' && (
          <div>
            <div className="p-5 border-b border-gray-100 flex gap-3">
              ...
              <Button onClick={() => { setDriverPage(1); loadDrivers(1); }}>Buscar</Button>
            </div>
            ...
            <Table>
              ...
              <Tbody>
                {loadingDrivers ? (
                  ...
                  <Tr><Td colSpan={6} className="text-center py-10 text-gray-400">No hay conductores</Td></Tr>
                ) : drivers.map(d => (
                  <Tr key={d.id}>
                    <Td>
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center shrink-0 overflow-hidden border border-blue-200">
                          {d.photo_url
                            ...
                            : <span className="text-sm font-semibold text-blue-700">{d.name.charAt(0)}</span>}
                        </div>
                        <div>
                    ...
                    <Td className="font-mono text-gray-600">{d.dni}</Td>
                    <Td className="text-gray-600">
                      {d.license_number ?? '—'}
                      {d.license_category && <span className="ml-1 text-xs text-gray-400">({d.license_category})</span>}
                    </Td>
                    <Td>
                      <span className={`text-sm ${expiryClass(d.license_expires_at)}`}>{fmtDate(d.license_expires_at)}</span>
                    </Td>
                    <Td>
                      <Badge className={`${DRIVER_STATUS_COLORS[d.status]} text-xs`}>{d.status}</Badge>
                    </Td>
                    <Td className="font-semibold text-gray-800">{d.reputation_score}</Td>
                  </Tr>
                ))}
              </Tbody>
            </Table>
            {driverTotalPages > 1 && (
              <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
                <p className="text-sm text-gray-500">Página {driverPage} de {driverTotalPages}</p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setDriverPage(p => p - 1)} disabled={driverPage === 1}>Anterior</Button>
                  <Button variant="outline" size="sm" onClick={() => setDriverPage(p => p + 1)} disabled={driverPage === driverTotalPages}>Siguiente</Button>
                </div>
              </div>
            )}
          </div>
        ...
        {tab === 'vehicles' && (
          <div>
            <div className="p-5 border-b border-gray-100 flex gap-3">
              ...
              <Button onClick={() => { setVehiclePage(1); loadVehicles(1); }}>Buscar</Button>
            </div>
            <Table>
              <Thead>
                <Tr>
              ...
              <Tbody>
                {loadingVehicles ? (
                  ...
                  <Tr><Td colSpan={7} className="text-center py-10 text-gray-400">No hay vehículos</Td></Tr>
                ) : vehicles.map(v => (
                  <Tr key={v.id}>
                    <Td>
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-10 rounded-md bg-gray-100 flex items-center justify-center shrink-0 overflow-hidden border border-gray-200">
                          {v.photo_url
                    ...
                    <Td>
                      <Badge className={`${VEHICLE_STATUS_COLORS[v.status]} text-xs`}>
                        {v.status.replace('_', ' ')}
                      </Badge>
                    </Td>
                    <Td>
                      <button onClick={() => setQrVehicle({ id: v.id, plate: v.plate })}
                        className="p-2 rounded-lg hover:bg-blue-50 text-[#2E86C1] transition-colors" title="Ver QR">
            ...
            {vehicleTotalPages > 1 && (
              <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
                <p className="text-sm text-gray-500">Página {vehiclePage} de {vehicleTotalPages}</p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setVehiclePage(p => p - 1)} disabled={vehiclePage === 1}>Anterior</Button>
                  <Button variant="outline" size="sm" onClick={() => setVehiclePage(p => p + 1)} disabled={vehiclePage === vehicleTotalPages}>Siguiente</Button>
                </div>
              </div>
            )}
          </div>
...
function InfoRow({ icon, label, value }: { icon: React.ReactNode; label: string; value?: string | null }) {
  return (
    <div className="flex items-start gap-3 py-2">
      <div className="p-2 bg-gray-50 rounded text-gray-500 shrink-0">{icon}</div>
      <div>
        <p className="text-xs text-gray-500 font-medium mb-0.5">{label}</p>
        <p className="text-sm font-medium text-gray-900">{value || <span className="text-gray-400 font-normal">No especificado</span>}</p>
      </div>
    </div>
  );
}
